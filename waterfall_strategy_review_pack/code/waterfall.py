...
def waterfall_settings(settings: dict[str, Any]) -> dict[str, Any]:
    ...
    cfg.setdefault("enabled_families", ["post_pump", "downtrend_continuation", "momentum_dump"])
...
class WaterfallEngine:
    ...
    def entry_signal(self, symbol: str, feat: dict[str, float], candle: Candle) -> tuple[WaterfallPosition, WaterfallSignal] | None:
        ...
        for rule in self.rules:
            ...
            pos = WaterfallPosition(
                ...
                entry_time=now,
                entry_price=entry,
                notional_usdt=float(self.cfg["notional_usdt"]),
                stop_price=stop,
                best_price=entry,
                worst_price=entry,
                trail_price=0.0,
            ...
            sig = WaterfallSignal(
                ...
                price=entry,
                stop_price=stop,
                confidence=confidence_from_features(feat),
                evidence=evidence,
            )
    ...
    def update_position(self, pos: WaterfallPosition, candle: Candle) -> WaterfallSignal | None:
        ...
        if candle.high >= pos.stop_price:
            ...
            trail_was_active = pos.trail_price > 0
            if not trail_was_active and mfe >= profile.trail_activate:
                ...
                if candle.high >= pos.trail_price:
            ...
            if not reason and mfe >= profile.rebound_activate:
                rebound = candle.close / pos.best_price - 1.0 if pos.best_price > 0 else 0.0
        ...
        pos.pnl_pct = pos.entry_price / exit_price - 1.0 - pos.fee_rate if exit_price > 0 else 0.0
...
async def waterfall_monitor(
    ...
    source = WebSocketMarketSource(settings, symbols, ["1m"])
    agen = source.events()
...
def render_waterfall_console(signal: WaterfallSignal) -> str:
    return (
        ...
        f"family={signal.family} rule={signal.rule}"
    )
...
def render_waterfall_wecom(signal: WaterfallSignal) -> str:
    ...
    if signal.action != "open_short":
        lines.append(f"> 收益 {signal.pnl_pct * 100:.2f}%")
    else:
        lines.append(f"> 置信 {signal.confidence:.3f}")
    return "\n".join(lines)
