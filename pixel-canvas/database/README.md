...
项目使用 SQLite，默认路径是 `backend/hunter/storage/hunter.db`。数据库由 `backend/hunter/pump_dump_hunter/data/store.py` 自动创建，`database/schema.sql` 是给人和后续 agent 阅读的结构说明。
...
```bash
sqlite3 backend/hunter/storage/hunter.db ".backup '/var/backups/hunter/hunter-$(date +%F).db'"
```
